...
def update_game_leaderboard(leaderboard_container, player_stats_container):
    ...
    if df is not None:
        ...
        view_df = display_df[["Rank", "Player", "Level"]]
            ...
            if not user_data.empty:
                ...
                st.markdown(f"""
                    <div style='background: #2D2D2D; padding: 15px; border-radius: 10px; text-align: center;'>
                        ...
                        <div style='font-size: 18px; margin: 5px 0;'>Level: {level}</div>
...
def admin_page():
    ...
    # Update question
        ...
        if levels:
            level_to_update = st.sidebar.selectbox("Select Level to Update", levels)
            ...
            if selected_question:
                ...
                with st.sidebar.form("update_question_form"):
                    ...
                    if st.form_submit_button("Update Question"):
                        ...
                        cursor.execute("""
                            UPDATE questions 
                            SET question = ?, answer = ?, hints = ?
                            WHERE level = ?
                        """, (new_question, new_answer, new_hints, level_to_update))
                        conn.commit()
                        conn.close()
                        st.success(f"Question for Level {level_to_update} updated successfully!")
                        st.rerun()
        else:
    ...
    # Delete question
        ...
        levels = [q[0] for q in questions]
        if levels:
            level_to_delete = st.sidebar.selectbox("Select Level to Delete", levels)
            if st.sidebar.button("Delete Question"):
                ...
                conn.commit()
                conn.close()
                st.success(f"Question for Level {level_to_delete} deleted successfully!")
                st.rerun()
        else:
    ...
    # Manage players
        ...
        if players:
            players_df = pd.DataFrame(players, columns=["Username", "Level"])
    ...
    if questions:
        questions_df = pd.DataFrame(questions, columns=["Level", "Question", "Answer", "Hints"])
...
def load_questions():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT level, question, answer, hints FROM questions ORDER BY level")
...
def get_current_leaderboard():
    ...
    if leaderboard:
        df = pd.DataFrame(leaderboard, columns=["Username", "Level", "Timestamp", "Position"])
        return df
    return None
...
# Streamlit app
st.markdown("<h1 style='color: white;'>Cryptic Hunt</h1>", unsafe_allow_html=True)
...
if st.session_state.username is None:
    ...
    # Add some space between leaderboard and login section
        ...
        if st.button("Login"):
            if username and password:
                ...
                if user:
                    ...
                    st.session_state.level = user[3]  # Current level
    ...
    time.sleep(2)  # Increased refresh interval to reduce flickering
    st.rerun()
    ...
    if current_level < len(questions):
        ...
        st.markdown(f"""
            <div class="question-card">
                <div class="level-title">Level {question_data[0]}</div>
